...
| ``nnz(A)``      | A 中非零元素的个数                                                   |
+-----------------+----------------------------------------------------------------------+
| ``size(A)``     | 返回一个包含 A 的维度的多元组                                        |
+-----------------+----------------------------------------------------------------------+
| ``size(A,n)``   | A 在某个维度上的长度                                                 |
+-----------------+----------------------------------------------------------------------+
| ``strides(A)``  | 返回多元组，其分量为在每个维度上，与邻接元素（在内存中）线性索引距离 |
+-----------------+----------------------------------------------------------------------+
| ``stride(A,k)`` | 在维度 k 上，与邻接元素（在内存中）的线性索引距离                    |
+-----------------+----------------------------------------------------------------------+
...
+---------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------+
| ``rand(dims)``                        | 在 [0,1) 上独立均匀分布的 ``Float64`` 类型的随机数组                                                                              |
+---------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------+
| ``randf(dims)``                       | 在 [0,1) 上均匀分布的 ``Float32`` 类型的随机数组                                                                                  |
+---------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------+
| ``randn(dims)``                       | ``Float64`` 类型的正态分布随机数组，均值为 0 ，标准差为 1                                                                         |
+---------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------+
| ``eye(n)``                            | n x n 单位矩阵                                                                                                                    |
+---------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------+
| ``eye(m, n)``                         | m x n 单位矩阵                                                                                                                    |
+---------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------+
| ``linspace(start, stop, n)``          | 从 ``start`` 至 ``stop`` 的 ``n`` 个元素的线性向量                                                                                |
+---------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------+
| ``fill!(A, x)``                       | 用 ``x`` 填充数组 ``A``                                                                                                           |
+---------------------------------------+-----------------------------------------------------------------------------------------------------------------------------------+
...
.. note:: 上例中， ``x`` 被声明为常量，因为对于非常量全局变量，Julia 的类型推断不怎么样。
...
数组支持下列运算符。在使用二元运算符时，如果两个输入都是向量，应使用带“点”版
本的运算符；如果其中一个输入是标量，两种版本的运算符都可以使用。
...
另外, Julia 提供了两个宏, ``@vectorize_1arg`` 和 ``@vectorize_2arg`` , 分别用
来向量化任意的一个或两个参数的函数. 每个宏接收两个参数, 函数的参数的类型和函数
的名字. 例如::
