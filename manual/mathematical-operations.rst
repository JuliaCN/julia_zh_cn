...
操作符 ``.<`` 是特别针对数组的; 只有当 ``A`` 和 ``B`` 有着相同的大小时,
``A .< B`` 才是合法的. 比较的结果是布尔型数组, 其大小同 ``A`` 和 ``B``
相同. 这样的操作符被称为 *按元素* 操作符; Julia 提供了一整套的按元素操
作符: ``.*``, ``.+``, 等等. 有的按元素操作符也可以接受纯量, 例如上一段
的 ``0 .< A .< B``. 这种表示法的意思是, 相应的纯量操作符会被施加到每一
个元素上去.
