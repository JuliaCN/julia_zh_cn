...
由于文本化的数组表达式在左方括号 ``[`` 与右方括号 ``]`` 匹配之前并不完整，可以如下来内插数组： ::
...
-  ``ind2chr(str,i)`` 给出 ``str`` 中递增至索引值 ``i`` 的字符数
-  ``chr2ind(str,j)`` 给出 ``str`` 中第 ``j`` 个字符的索引值
...
Julia 的字符串支持内插和转义。而非标准字符串文本，可以关闭其中的某个特性：

-  ``E"..."`` 允许转义，但是不允许内插，因而 ``$`` 可当做普通字符使用
-  ``I"..."`` 允许内插，但是不转义
-  ``L"..."`` 既不允许转义，也不允许内插

如果你要写含有很多 ``$`` 字符的字符串，你可以用反斜杠将其转义，但是这有些繁琐。这时就可以使用 ``E`` 前缀的字符串文本： ::
...
数字 ``100`` 被内插为字符串 ``"100"`` ，但 ``\n`` 没有被转义为新行。

``L"..."`` 字符串文本既不允许转义，也不允许内插： ::
...
在字节数组文本中，对象被内插为二进制表达式，而不是字符串表达式： ::
