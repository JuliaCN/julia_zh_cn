...
("杂项","Base","edit","edit(file::String[, line])
   ...
   编辑文件；可选择性地提供要编辑的行号。退出编辑器后返回 Julia 会话。 如果文件后缀名为 \".jl\"
   ，关闭文件后会重载该文件。
...
("杂项","Base","include","include(path::String)
   ...
   在当前上下文中，对源文件的内容求值。在包含的过程中，它将本地任务 包含的路径设置为包含文件的文件夹。嵌套调用 \"include\"
   时会搜索 那个路径的   相关路径。并行运行时，所有的路径都指向节点 1 上文件， 并从节点 1
   上获取文件。此函数常用来交互式地载入源文件，或将分散为 多个源文件的扩展包结合起来。
...
("杂项","Base","methodswith","methodswith(typ[, showparents])
   ...
   显示 \"typ\" 类型的所有方法。若可选项 \"showparents\" 为 \"true\" ， 则额外显示 \"typ\"
   除 \"Any\" 类型之外的父类型的方法。
...
("所有对象","Base","deepcopy","deepcopy(x)
   ...
   dict::ObjectIdDict)\" 函数（此函数其它情况下 不应使用）来覆盖默认的 \"deepcopy\" 行为，其中
...
("通用函数","Base","invoke","invoke(f, (types...), args...)
   ...
   对指定的参数，为匹配指定类型（多元组）的通用函数指定要调用的方法。 参数应与指定的类型兼容。它允许在最匹配的方法之外，指定一个方法。
   这对明确需要一个更通用的定义的行为时非常有用 （通常作为相同函数的更特殊的方法实现的一部分）。
...
("迭代","Base","zip","zip(iters...)
   ...
   对一组迭代对象，返回一组可迭代多元组，其中第 \"i\" 个多元组 包含每个可迭代输入的第 \"i\" 个分量。
...
("可迭代集合","Base","reduce","reduce(op, v0, itr)
   ...
   使用指定的运算符约简指定集合， \"v0\" 为约简的初始值。一些常用运算符的缩减， 有更简便的单参数格式：
   \"max(itr)\", \"min(itr)\", \"sum(itr)\", \"prod(itr)\",
   \"any(itr)\", \"all(itr)\".
...
("字符串","Base","split","split(string, [chars, [limit,] [include_empty]])
   ...
   返回由指定字符分割符所分割的指定字符串的字符串数组。 分隔符可由 \"search\" 的第二个参数所允许的任何格式所指明（如单字符、
   字符集合、字符串、或正则表达式）。如果省略 \"chars\" ， 则它默认为整个空白字符集，且 \"include_empty\"
...
("文本 I/O","Base","readdlm","readdlm(filename, delim::Char)
   ...
   如果所有的数据都是数值，结果为数值矩阵。如果有些元素不能被解析为数， 将返回由数和字符串构成的元胞数组。

"),

("文本 I/O","Base","readdlm","readdlm(filename, delim::Char, T::Type)

   从文本文件中读取指定元素类型的矩阵。如果 \"T\" 是数值类型，结果为此类型的数组： 若为浮点数类型，非数值的元素变为
   \"NaN\" ；其余类型为 0 。 \"T\" 的类型还有 \"ASCIIString\", \"String\", 和
   \"Any\" 。
...
("内存映射 I/O","Base","msync","msync(array)
   ...
   对内存映射数组的内存中的版本和硬盘上的版本强制同步。 程序员可能不需要调用此函数，因为操作系统在休息时自动同步。但是，
...
("数学函数","Base","dawson","dawson(x)
   ...
   计算 \"x\" 的 Dawson 函数（缩放虚误差函数）， 其定义为 \\frac{\\sqrt{\\pi}}{2}
   e^{-x^2} \\operatorname{erfi}(x).
...
("数学函数","Base","factor","factor(n)
   ...
   对 \"n\" 分解质因数，返回一个字典。 字典的键对应于质因数，与 \"n\" 类型相同。
...
("数据格式","Base","base","base(base, n[, pad])
   ...
   将整数 \"n\" 转换为指定进制 \"base\" 的字符串。 可选择性指明空白补位后的位数。进制 \"base\" 可以为整数，
...
("随机数","Base","srand","srand([rng], seed)
   ...
   使用 \"seed\" 为 RNG 的种子，可以是无符号整数或向量。 \"seed\" 也可以是文件名，此时从文件中读取种子。
...
("数组","Base","permutedims","permutedims(A, perm)
   ...
   重新排列数组 \"A\" 的维度。 \"perm\" 为长度为 \"ndims(A)\" 的向量，它指明如何排列。
...
("数组","Base","mapslices","mapslices(f, A, dims)
   ...
   上都调用函数 \"f\" 。 整数向量 \"dims\" 指明了维度信息。结果将沿着未指明的维度进行连接。 例如，如果
   \"dims\" 为 \"[1,2]\" ， \"A\" 是四维数组， 此函数将对每个 \"i\" 和 \"`j` 调用 ``f\"
   处理 \"A[:,:,i,j]\" 。
...
("统计","Base","hist","hist(v[, n]) -> e, counts
   ...
   计算 \"v\" 的直方图，可以指定划大致分为 \"n\" 个区间。 返回值为范围 \"e\" ，对应于区间的边缘，
...
("统计","Base","hist","hist(v, e) -> e, counts
   ...
   结果为长为 \"length(e)-1\" 的向量，且第 \"i\" 个元素满足 \"sum(e[i] .< v .<=
   e[i+1])\"
...
("统计","Base","cor","cor(v1[, v2])
   ...
   计算两个向量 \"v1\" 和 \"v2\" 的 Pearson 相关系数。 如果调用时只有 \"v\" 这一个参数，它计算
   \"v\" 中列的相关系数。
...
("信号处理","Base","rfft","rfft(A[, dims])
   ...
   对实数数组 \"A\" 做多维 FFT 。由于转换具有共轭对称性，相比 \"fft()\" ， 可节约将近一半的计算时间和存储空间。
   如果 \"A\" 的大小为 \"(n_1, ..., n_d)\" ， 结果的大小为 \"(floor(n_1/2)+1, ...,
   n_d)\" 。
...
("信号处理","Base","plan_irfft","plan_irfft(A, d[, dims[, flags[, timelimit]]])
   ...
   制定优化实数输入 FFT 的方案。 与 \"plan_rfft()\" 类似，但它对应于 \"irfft()\" 。
   前三个参数的意义与 \"irfft()\" 相同。
...
("信号处理","Base","plan_brfft","plan_brfft(A, d[, dims[, flags[, timelimit]]])
   ...
   制定优化实数输入 FFT 的方案。 与 \"plan_rfft()\" 类似，但它对应于 \"brfft()\" 。
   前三个参数的意义与 \"brfft()\" 相同。
...
("信号处理","Base","dct","dct(A[, dims])
   ...
   指明了关于维度的可迭代集合 （如整数、范围、多元组、数组）。 如果 \"A\" 要运算的维度上的长度是较小的质数的积，算法会比较高效；
   详见 \"nextprod()\" 。另见高效的 \"plan_dct()\" 。
...
("信号处理","Base","idct","idct(A[, dims])
   ...
   指明了关于维度的可迭代集合（如整数、范围、 多元组、数组）。如果 \"A\" 要运算的维度上的长度是较小的质数的积，
   算法会比较高效；详见 \"nextprod()\" 。另见高效的 \"plan_idct()\" 。
...
("信号处理","Base","FFTW","FFTW.r2r(A, kind[, dims])
   ...
   \"FFTW.R2HC\" 及它的逆 \"FFTW.HC2R\")，或离散 Hartley 变换（ \"FFTW.DHT\" ）。
   参数 \"kind\" 可以为数组或多元组， 可用来指明在 \"A\" 的不同维度上做不同种类的变换； 对未指明的维度使用
...
("C 接口","Base","dlopen","dlopen(libfile::String[, flags::Integer])
   ...
   它们被转换为对应的 POSIX dlopen 命令的标志位； 如果当前平台不支持某个特性，则忽略。 默认值为
...
("线性代数","","\\","\\(A, B)
   ...
   分解。对长方矩阵 \"A\" ，通过将 \"A\" 约简到双对角线形式， 然后使用最小范数最小二乘法来求解双对角线最小二乘问题。
...
("线性代数","","lufact!","lufact!(A) -> LUDense
   ...
   \"nzval\" 域，仅覆写索引值域， \"colptr\" 和 \"rowval\" 在原地缩减， 使得从 1 开始的索引改为从
   0 开始的索引。
...
("线性代数","","qrpfact","qrpfact(A) -> QRPivotedDense
   ...
   \"Q\" 为 \"F[:Q]\" ， 三角矩阵 \"R\" 为 \"F[:R]\" ，置换矩阵和置换向量分布为 \"F[:P]\"
   和 \"F[:p]\" 。 \"QRDensePivoted\" 对象可使用下列函数： \"size\", \"\\\" 。 提取的
...
("线性代数","","hessfact","hessfact(A)
   ...
   对 \"A\" 做 Hessenberg 分解，返回 \"HessenbergDense\" 对象。 如果分解后的结果为 \"F\"
   ，酉矩阵为 \"F[:Q]\" ， Hessenberg 矩阵为 \"F[:H]\" 。 提取的 \"Q\" 是
   \"HessenbergDenseQ\" 对象， 可以使用 \"full\" 函数将其转换为普通矩阵。
...
("BLAS 函数","","copy!","copy!(n, X, incx, Y, incy)
   ...
   将内存邻接距离为 \"incx\" 的数组 \"X\" 的 \"n\" 个元素复制到 内存邻接距离为 \"incy\" 的数组
   \"Y\" 中。返回 \"Y\" 。
...
("BLAS 函数","","syrk!","syrk!(uplo, trans, alpha, A, beta, C)
   ...
   \"alpha*A.'*A + beta*C\" ，由参数 \"uplo\" （ 'U' 或 'L' ）确定，
...
("BLAS 函数","","herk!","herk!(uplo, trans, alpha, A, beta, C)
   ...
   beta*C\" 或 \"alpha*A'*A + beta*C\" ， 由参数 \"uplo\" （ 'U' 或 'L' ）确定，
   用计算的结果更新对称矩阵 \"C\" 的上三角矩阵或下三角矩阵。返回 \"C\" 。
